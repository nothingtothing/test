...
def spamTest():
    ...
    df = pd.read_csv("../data/result_process01",encoding="utf-8",sep=",") #如果有nan值，进行上删除操作
    df.dropna(axis=0,how="any",inplace=True)    #删除表中含有任何NaN的行
    df.columns = ['a','b','c','content','classes']
    docList = [df.content.ix[i].split() for i in range(204)]
    classList = [df.classes.ix[i] for i in range(204)]
    vocabList = createVocabList(docList)  # 创建词汇表，不重复
    trainingSet = list(range(204))

    testSet = []  # 创建存储训练集的索引值的列表和测试集的索引值的列表
    for i in range(40):  # 从50个邮件中，随机挑选出40个作为训练集,10个做测试集
